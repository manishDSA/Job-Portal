...
// Only initialize Firebase SDK if a valid API key environment variable is configured
...
export const signInWithSocialProvider = async (providerName) => {
    // If Firebase is initialized with a valid API key, trigger Firebase popup auth
    if (auth) {
        let provider;
    ...
    // Smooth fallback for local development without making failing network calls to identitytoolkit
